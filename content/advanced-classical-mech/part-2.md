...
We know from Newtonian mechanics that there exist certain **conserved quantities**, most particularly energy, momentum, and angular momentum. We will now show how these follow naturally in Lagrangian mechanics if we take the Lagrangian to satisfy some specific conditions, and thus resultingly have a Lagrangian in a particular form. It is the result of one of the most famous theorems of physics: **Noether's theorem**, named after the great mathematician [Emmy Noether](https://en.wikipedia.org/wiki/Emmy_Noether), who discovered it in 1918.

Noether found that the Lagrangian approach to mechanics is far more than just a mathematical trick; in actuality, the Lagrangian holds deep (but hidden) physical meaning. This comes from the fact that a Lagrangian that meets certain **constraints** (which we'll interchangeably call _conditions_), such as time-independence or position-independence, won't change when we transform a system in a particular way. These constraints are more commonly called **symmetries** by physicists, since they tell us that the Lagrangian doesn't change when certain transformations (like rotations and translations) are applied to it. Just like the inherent symmetry of snowflakes means they look the same when turned 180 degrees around, the symmetries of certain Lagrangians leads them to have the same mathematical form _both before and after_ a transformation (in physics language, this is called **invariance**). Noether's genius was in proving that Lagrangians that obey symmetries _must_ have associated conserved quantities that can be _precisely_ found from these symmetries, leading to **all** the conservation laws we see in physics. While we won't prove Noether's theorem at the level of formal mathematical rigor, we'll retrace Noether's steps and show how we can arrive at her groundbreaking results.

> **Note:** If you have time, I highly recommend reading more about Noether; she is certainly one of greatest mathematicians in history and her story overcoming stiff resistance as a female and Jewish mathematician in the 20th century makes it all the more impressive.

### Energy conservation

To start, let us consider a Lagrangian describing some physical system. For energy to be conserved in the system, its Lagrangian must meet **four conditions**, which are interrelated:
