...
To start, we pick $b_1 = 1$, which is the simplest possibility. While this is not necessarily a rigorous argument by itself, a simple theory is almost always more preferred than a more complex theory in physics. Indeed, it does also help that by setting $b_1 = 1$ we get the correct expression of the gravitational force, which can be deduced through basic experimental observations and some [clever reasoning](https://www.youtube.com/watch?v=FfRcRm4oO8E). In choosing this, the zeroeth-order term of the potential becomes the familiar potential $\Phi(r) = -\dfrac{GM}{r}$, so we (retroactively) know this choice to be correct.

But what about the higher-order terms? The answer comes from the observation that $G$ is already a very small number, and the $r^{-n}$ dependence of the terms means that gravity rapidly decreases with distance. This means that the $r^{-3}$ term is suppressed (near-zero) for all but very strong gravitational fields. This is important because we'd expect General Relativity to be the correct theory of gravity in the strong-field regime, and thus the $b_3$ term is suppressed in weak fields (as in Newtonian gravity) and only becomes relevant in GR. Thus, Newtonian gravity is the _effective field theory_ of gravity, since whenever we have $b_3 \ll 1$ (corresponding to weak gravitational fields) all terms above first-order vanish, reducing to:
...
> **Note:** In this section, I will be discussing _perturbative quantum gravity_, which is an [effective field theory](https://en.wikipedia.org/wiki/Effective_field_theory) of quantum gravity, and is generally agreed to be understood (within the framework of quantum field theory). This effective field theory of quantum gravity should hold up from "everyday" energies all the way to a few orders of magnitude below the [Planck scale](https://en.wikipedia.org/wiki/Planck_units#Planck_scale). I will **not** be discussing quantum gravity at energies close to the Planck scale, where we don't understand how gravity works at all, and it is generally agreed that we need some completely new theory to describe physics at such extreme conditions.
